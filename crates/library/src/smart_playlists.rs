...
use crate::{AlbumArtwork, Track, msgid};
...
mod tests {
    ...
    #[test]
    fn nested_rules_and_ranges_are_normalized() {
        ...
        let SmartPlaylistRuleNode::Rule(year) = &group.rules[0] else {
            panic!("year rule");
        };
        assert_eq!(
            year.value,
        ...
        let SmartPlaylistRuleNode::Group(nested) = &group.rules[1] else {
            panic!("nested group");
        };
        let SmartPlaylistRuleNode::Rule(genre) = &nested.rules[0] else {
            panic!("genre rule");
        };
        assert_eq!(
            genre.value,
        ...
        let SmartPlaylistRuleNode::Rule(date) = &group.rules[2] else {
            panic!("date rule");
        };
        assert_eq!(
            date.value,
    ...
    #[test]
    fn invalid_empty_rule_is_dropped_but_empty_root_is_allowed() {
        let mut group = SmartPlaylistRuleGroup {
            mode: SmartPlaylistMatchMode::All,
            rules: vec![SmartPlaylistRuleNode::Rule(SmartPlaylistRule {
        ...
        assert!(normalize_group(&mut group).is_none());
