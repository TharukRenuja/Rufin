...
fn print_usage() {
    eprintln!(
        "Usage:
  cargo xtask generate flatpak-sources [--check]
  cargo xtask generate i18n-template [--check] [--output PATH]
  cargo xtask generate nix-cargo-hash [--check]
  cargo xtask generate aur-stable [--check] [--skip-srcinfo] VERSION
  cargo xtask release prepare VERSION SUMMARY
  cargo xtask release create-tag [--base TAG] [--dry-run] [--replace] [--skip-flathub] VERSION SUMMARY
  cargo xtask release update-flathub-manifest [--manifest PATH] TAG
  cargo xtask verify icons
  cargo xtask verify local [--base-ref REF]
  cargo xtask verify package-layout ROOT [PREFIX]
