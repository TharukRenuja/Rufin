...
pub(crate) struct ColumnViewWidthFit {
    ...
    columns: Rc<RefCell<Vec<(gtk::ColumnViewColumn, i32)>>>,
    fallback_width: i32,
    revision: Rc<Cell<u64>>,
    last_fit: Rc<Cell<Option<(i32, u64)>>>,
}
...
impl ColumnViewWidthFit {
    ...
    pub(crate) fn fit_scroller_allocation(
        ...
        let fit = (available_width, self.revision.get());
        if self.last_fit.replace(Some(fit)) == Some(fit) {
            return;
        }

        fit_column_widths(&self.columns.borrow(), available_width);
    }
}
...
pub(crate) fn install_column_view_width_fit(
    ...
    fit_column_widths(&columns.borrow(), initial_width);
    ColumnViewWidthFit {
        table: table.downgrade(),
        columns,
        fallback_width: initial_width,
        revision: Rc::new(Cell::new(0)),
        last_fit: Rc::new(Cell::new(Some((initial_width, 0)))),
    }
}
