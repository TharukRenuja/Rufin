...
impl Shell {
    ...
    pub(crate) fn update_mpris_player_after(&self, discontinuity: Option<PositionDiscontinuity>) {
        ...
        self.desktop.mpris.queue(desired, discontinuity);
    }
...
pub(crate) fn install_mpris(shell: &Rc<Shell>) {
    ...
    glib::spawn_future_local(async move {
        ...
        player.connect_set_position(move |_, track_id, position| {
            let current_matches = mpris_set_position_matches(
                position_shell.playback.player.borrow().as_ref(),
                track_id,
            );
            if current_matches {
                position_shell
        ...
        shell.desktop.mpris.install_player(Rc::clone(&player));
...
mod tests {
    use super::{
        RepeatMode, SequenceEntry, TransportStatus, mpris_desired_state, mpris_loop_status,
        mpris_metadata, mpris_set_position_matches, mpris_track_id, repeat_mode_from_mpris,
    };
    use crate::shell::shell_tests::{test_image_ref, test_playback_view, test_sequence_entry};
