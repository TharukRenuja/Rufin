...
use crate::{ExternalLyricsProvider, LyricLine, Lyrics, LyricsSearchResult, LyricsSource};
...
pub const REMOTE_LYRICS_ORIGIN: &str = "remote";

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum LyricsRequestKind {
    Automatic,
    Configured,
    ServerOnly,
}
...
impl crate::Settings {
    pub fn lyrics_plan(&self, private_mode: bool, kind: LyricsRequestKind) -> LyricsPlan {
        let external_allowed = self.external_lyrics_allowed(private_mode);
        let (native_search, allow_external_fallback) = match kind {
            LyricsRequestKind::Automatic => (sources::LyricsSearch::ServerOnly, external_allowed),
            LyricsRequestKind::Configured if external_allowed && self.prefer_server_lyrics => {
                (sources::LyricsSearch::ServerThenRemote, true)
            }
            LyricsRequestKind::Configured if external_allowed => {
                (sources::LyricsSearch::RemoteThenServer, true)
            }
            LyricsRequestKind::Configured | LyricsRequestKind::ServerOnly => {
                (sources::LyricsSearch::ServerOnly, false)
            }
        };
        LyricsPlan {
            native_search,
            external_providers: self.external_lyrics_providers.clone(),
            allow_external_fallback,
        }
    }

    pub fn automatic_lyrics_request(
        &self,
        private_mode: bool,
        track_id: &TrackId,
    ) -> LyricsRequestKind {
        if !self.external_lyrics_allowed(private_mode) || self.auto_lyrics_suppressed(track_id) {
            LyricsRequestKind::ServerOnly
        } else {
            LyricsRequestKind::Automatic
        }
    }
    ...
    pub fn can_suppress_auto_lyrics(
        ...
        lyrics.is_some_and(|lyrics| {
            lyrics.track_id == *track_id
                && lyrics.source == LyricsSource::Remote
                && self.external_lyrics_allowed(private_mode)
...
pub struct LyricsResolution {
    ...
    pub cache: LyricsCacheUpdate,
}

#[derive(Clone, Debug)]
pub struct ResolveLyrics {
    pub track: Track,
    pub plan: LyricsPlan,
    pub use_cache: bool,
    pub cue_track: bool,
    pub local: Option<LocalLyricsInput>,
    pub cached: Option<Lyrics>,
}

pub fn resolve_lyrics(
    request: ResolveLyrics,
    native: impl FnOnce(sources::LyricsSearch) -> Result<Option<sources::NativeLyrics>, String>,
    is_current: impl Fn() -> bool,
) -> LyricsResolution {
    if !is_current() {
        return LyricsResolution {
            lyrics: None,
            cache: LyricsCacheUpdate::None,
        };
    }
    if let Some(input) = request.local.as_ref()
        && let Some(lyrics) = local_sidecar_lyrics(&request.track.id, input)
    {
        return LyricsResolution {
            lyrics: Some(lyrics.clone()),
            cache: LyricsCacheUpdate::Save(lyrics),
        };
    }

    let mut cache = LyricsCacheUpdate::None;
    if request.use_cache
        && let Some(cached) = request.cached
        && cached_lyrics_allowed(&cached, &request.plan, request.cue_track)
    {
        let remote = cached.source == LyricsSource::Remote;
        ...
        if remote {
            cache = LyricsCacheUpdate::DeleteRemote;
        }
    }

    match native(request.plan.native_search) {
        Ok(Some(native)) => {
            let lyrics = lyrics_from_native(request.track.id, native);
            return LyricsResolution {
                lyrics: Some(lyrics.clone()),
                cache: LyricsCacheUpdate::Save(lyrics),
            };
        ...
        Ok(None) => {}
        Err(error) => {
            tracing::warn!(track_id = %request.track.id, %error, "native lyrics request failed");
        }
    }

    if request.plan.allow_external_fallback && is_current() {
        match external_best_lyrics(&request.track, &request.plan.external_providers) {
            Ok(Some(lyrics)) => {
                return LyricsResolution {
                    lyrics: Some(lyrics.clone()),
                    cache: LyricsCacheUpdate::Save(lyrics),
            ...
            Ok(None) => {}
            Err(error) => {
                tracing::debug!(track_id = %request.track.id, %error, "external lyrics request failed");
            }
        }
    }
...
pub fn encode_cached_lyrics(lyrics: &Lyrics) -> Result<(&'static str, String), String> {
    let origin = match lyrics.source {
        LyricsSource::Local => "local",
        LyricsSource::Server => "server",
        LyricsSource::Remote => REMOTE_LYRICS_ORIGIN,
    };
    serde_json::to_string(lyrics)
        .map(|payload| (origin, payload))
        .map_err(|error| error.to_string())
}

pub fn decode_cached_lyrics(payload: &str) -> Result<Lyrics, String> {
    serde_json::from_str(payload).map_err(|error| error.to_string())
}

fn lyrics_from_native(track_id: TrackId, native: sources::NativeLyrics) -> Lyrics {
    Lyrics {
        track_id,
        source: match native.origin {
            sources::NativeLyricsOrigin::Server => LyricsSource::Server,
            sources::NativeLyricsOrigin::Remote => LyricsSource::Remote,
        },
...
fn cached_lyrics_allowed(lyrics: &Lyrics, plan: &LyricsPlan, cue_track: bool) -> bool {
    let allowed = match lyrics.source {
        LyricsSource::Local | LyricsSource::Server => true,
        LyricsSource::Remote => {
            plan.allow_external_fallback
                && lyrics
                    .external_provider
                    .is_none_or(|provider| plan.external_providers.contains(&provider))
        }
    };
    allowed && !(cue_track && lyrics.source == LyricsSource::Local)
}

fn lyrics_from_text_content(
    track_id: TrackId,
...
fn lyrics_with_displayable_content(mut lyrics: Lyrics) -> Option<Lyrics> {
...
fn external_best_lyrics(
    ...
    for result in results {
        match lyrics_from_search_result(track.id.clone(), &result) {
...
pub fn lyrics_from_search_result(
    track_id: TrackId,
...
fn local_sidecar_lyrics(track_id: &TrackId, input: &LocalLyricsInput) -> Option<Lyrics> {
...
mod tests {
    ...
    use library::{AlbumId, ArtistCredit, ArtistId};
    ...
    fn track() -> Track {
        Track {
            ...
            album_id: AlbumId::new("jellyfin:album:disintegration"),
            title: "Lovesong".to_string(),
            artist: "The Cure • Robert Smith".to_string(),
            artist_id: Some(ArtistId::new("jellyfin:artist:the-cure")),
    ...
    #[test]
    fn plans_preserve_preference_and_private_mode() {
        let mut settings = crate::Settings::default();
        let plan = settings.lyrics_plan(false, LyricsRequestKind::Configured);
        ...
        let plan = settings.lyrics_plan(true, LyricsRequestKind::Configured);
        assert_eq!(plan.native_search, sources::LyricsSearch::ServerOnly);
        assert!(!plan.allow_external_fallback);

        let plan = settings.lyrics_plan(false, LyricsRequestKind::ServerOnly);
        assert_eq!(plan.native_search, sources::LyricsSearch::ServerOnly);
        assert!(!plan.allow_external_fallback);

        let plan = settings.lyrics_plan(false, LyricsRequestKind::Automatic);
        assert_eq!(plan.native_search, sources::LyricsSearch::ServerOnly);
        assert!(plan.allow_external_fallback);

        let track_id = track().id;
        assert_eq!(
            settings.automatic_lyrics_request(false, &track_id),
            LyricsRequestKind::Automatic
        );
    ...
    #[test]
    fn resolution_reuses_allowed_cache_and_rejects_cue_local_cache() {
        let mut settings = crate::Settings::default();
        settings.external_lyrics_providers = vec![ExternalLyricsProvider::Genius];
        settings.prefer_server_lyrics = false;
        let remote = Lyrics {
            track_id: track().id,
            source: LyricsSource::Remote,
            external_provider: Some(ExternalLyricsProvider::Genius),
            lines: vec![line("cached")],
        };
        let native_called = Cell::new(false);
        let resolution = resolve_lyrics(
            ResolveLyrics {
                track: track(),
                plan: settings.lyrics_plan(false, LyricsRequestKind::Configured),
                use_cache: true,
                cue_track: false,
                local: None,
        ...
        assert_eq!(resolution.lyrics, Some(remote));
        assert_eq!(resolution.cache, LyricsCacheUpdate::None);
        ...
        let local = Lyrics {
            ...
            source: LyricsSource::Local,
            external_provider: None,
            lines: vec![line("wrong cue lyrics")],
        };
        let resolution = resolve_lyrics(
            ResolveLyrics {
                track: track(),
                plan: settings.lyrics_plan(true, LyricsRequestKind::ServerOnly),
                use_cache: true,
                cue_track: true,
                local: None,
                cached: Some(local),
            ...
            |_| {
                Ok(Some(sources::NativeLyrics {
                    origin: sources::NativeLyricsOrigin::Server,
        ...
        assert_eq!(
            ...
            Some(LyricsSource::Server)
        );
        assert!(matches!(resolution.cache, LyricsCacheUpdate::Save(_)));
    }

    #[test]
    fn cancelled_resolution_starts_no_provider_work() {
        let native_called = Cell::new(false);
        let resolution = resolve_lyrics(
            ResolveLyrics {
                track: track(),
                plan: crate::Settings::default().lyrics_plan(false, LyricsRequestKind::Configured),
                use_cache: true,
                cue_track: false,
                local: None,
                cached: None,
        ...
        assert_eq!(resolution.lyrics, None);
        assert_eq!(resolution.cache, LyricsCacheUpdate::None);
        assert!(!native_called.get());
    }
    ...
    fn local_sidecar_uses_title_for_cue_and_rejects_oversized_files() {
        ...
        let lyrics = local_sidecar_lyrics(&track().id, &input).expect("lyrics");
        assert_eq!(
            lyrics.lines,
            vec![LyricLine {
    ...
    fn save_uses_the_selected_path_and_rejects_empty_provider_content() {
        ...
        let (path, lyrics) = save_lyrics_search_result(track().id, &result, output.clone())
            .expect("save")
            .expect("lyrics");
        ...
        assert_eq!(
            save_lyrics_search_result(
                track().id,
                &placeholder,
                directory.path().join("placeholder.lrc")
            )
            .expect("placeholder"),
            None
        );
