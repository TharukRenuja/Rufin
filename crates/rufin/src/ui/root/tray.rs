...
pub(super) type TrayHandle = ksni::blocking::Handle<RufinTray>;

#[derive(Clone)]
pub(super) struct RufinTray {
...
pub(super) fn install_tray(shell: &Rc<Shell>) {
    if shell.state.settings.borrow().tray_enabled {
        shell.ensure_tray();
    }

    let close_shell = Rc::clone(shell);
    shell.window.connect_close_request(move |_| {
        let settings = close_shell.state.settings.borrow().clone();
...
pub(super) fn present_initial_window(shell: &Rc<Shell>) {
...
impl Shell {
    pub(super) fn set_tray_enabled(self: &Rc<Self>, enabled: bool) {
        if self
    ...
    pub(super) fn set_exit_to_tray_enabled(self: &Rc<Self>, enabled: bool) {
        if self
    ...
    pub(super) fn set_start_minimized_enabled(self: &Rc<Self>, enabled: bool) {
        if self
    ...
    fn ensure_tray(self: &Rc<Self>) -> bool {
        if self.state.tray_handle.borrow().is_some() {
            return true;
        }

        let (sender, receiver) = channel();
        let tray = RufinTray::new(sender, self.state.settings.borrow().private_mode);
        ...
        *self.state.tray_handle.borrow_mut() = Some(handle);
        self.install_tray_command_pump(receiver);
        true
    }

    fn shutdown_tray(&self) {
        if let Some(source) = self.state.tray_command_source.borrow_mut().take() {
            source.remove();
        }
        if let Some(handle) = self.state.tray_handle.borrow_mut().take() {
            handle.shutdown().wait();
        }
    }

    pub(super) fn refresh_tray_private_mode(&self) {
        let private_mode = self.state.settings.borrow().private_mode;
        if let Some(handle) = self.state.tray_handle.borrow().as_ref().cloned() {
    ...
    fn install_tray_command_pump(self: &Rc<Self>, receiver: Receiver<TrayCommand>) {
            ...
            glib::timeout_add_local(Duration::from_millis(TRAY_POLL_INTERVAL_MS), move || {
                while let Ok(command) = receiver.try_recv() {
                    match command {
                        ...
                        TrayCommand::PlayPause => shell.controller.play_pause(),
                        TrayCommand::PreviousTrack => shell.controller.previous_track(),
                        TrayCommand::NextTrack => shell.controller.next_track(),
                        TrayCommand::TogglePrivateMode => {
                            let enabled = !shell.state.settings.borrow().private_mode;
                            shell.set_private_mode(enabled);
                        }
                        TrayCommand::Quit => {
                            shell.shutdown_tray();
                            shell.application.quit();
        ...
        *self.state.tray_command_source.borrow_mut() = Some(source);
    }
...
fn build_tray_icon_pixmaps() -> Vec<ksni::Icon> {
    let source = match Pixbuf::from_read(Cursor::new(APP_ICON_BYTES)) {
        Ok(source) => source,
        Err(error) => {
    ...
    TRAY_ICON_SIZES
        ...
        .filter_map(|size| {
            source
                ...
                .and_then(|pixbuf| tray_icon_from_pixbuf(&pixbuf))
        })
        .collect()
}

fn tray_icon_from_pixbuf(pixbuf: &Pixbuf) -> Option<ksni::Icon> {
    let width = pixbuf.width();
    let height = pixbuf.height();
    let channels = pixbuf.n_channels();
    if width <= 0 || height <= 0 || channels < 3 || pixbuf.bits_per_sample() != 8 {
        return None;
    }
    ...
    let channels_usize = usize::try_from(channels).ok()?;
    ...
    for y in 0..height_usize {
        ...
        for x in 0..width_usize {
            let offset = row.checked_add(x.checked_mul(channels_usize)?)?;
            let end = offset.checked_add(channels_usize)?;
            let pixel = pixels.get(offset..end)?;
            let alpha = if pixbuf.has_alpha() && channels_usize >= 4 {
                pixel[3]
            } else {
                u8::MAX
            };
